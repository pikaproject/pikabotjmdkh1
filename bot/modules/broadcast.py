...
async def broadcast(bot, message):
    ...
    if not config_dict['DATABASE_URL']:
        await bot.send_message(chat_id=message.chat.id, text="DATABASE_URL not provided")
    else:
        ...
        for chat_id in chat_ids:
            try:
                await bot.copy_message(chat_id=chat_id, from_chat_id=message.chat.id, message_id=message.reply_to.message_id)
                success += 1
