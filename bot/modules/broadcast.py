...
async def broadcast(client, message):
    ...
    if not config_dict['DATABASE_URL']:
        ...
        for chat_id in chat_ids:
            try:
                if auth := True:
                   return await client.copy_message(chat_id=chat_id, from_chat_id=message.chat.id, reply_to_message_id=message.id)#reply_message_id = message.reply_to_message.message_id #message_id=reply_to_message.id)
                   success += 1
                else:
